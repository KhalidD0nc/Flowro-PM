...
import { getPreviewPort, isServerReady, startDevServer } from "@/lib/build-worker/fs"
...
export async function POST(
    ...
    try {
        ...
        const successRun = runs.find((r) => r.status === "success" && r.previewAvailable && r.targetWorkspacePath)

        if (!successRun?.targetWorkspacePath) {
            return NextResponse.json({ error: "No successful build found" }, { status: 404 })
        }
        ...
        if (!alive) {
            startDevServer(successRun.targetWorkspacePath, projectId, port)
            // Give the server a moment to bind before the client polls
            await new Promise((r) => setTimeout(r, 800))
        }

        return NextResponse.json({ previewUrl, wasRestarted: !alive })
    } catch (error) {
