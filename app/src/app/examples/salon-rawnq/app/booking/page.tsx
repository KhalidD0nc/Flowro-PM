...
import { Button, Input, Label, Panel, PageHeader, FadeIn, StatusBadge } from "@/components/examples/salon-rawnq/app-kit";
import {
  mockServices, mockAvailabilitySlots, mockStaffMembers, type Booking as BookingType,
} from "@/lib/examples/salon-rawnq/mock-data";
...
function BookingWizard() {
  ...
  const availableSlots = selectedDate
    ? mockAvailabilitySlots.filter((s) => s.date === selectedDate && s.isAvailable)
    : [];
      ...
      <div className="max-w-2xl mx-auto py-12 text-center space-y-8">
        <FadeIn>
          ...
          <Panel className="text-start border-line p-8 md:p-10 space-y-6">
            <div className="flex justify-between items-center border-b border-line pb-6">
              <div>
                ...
                <p className="font-mono font-bold text-primary text-xl">
                  #{confirmedBooking.id.split("_")[1].toUpperCase()}
                </p>
              </div>
            ...
            <div className="grid gap-6 sm:grid-cols-2 pt-2">
              ...
              <div>
                ...
                <p className="font-bold text-ink text-lg">{confirmedBooking.price} ريال</p>
              </div>
              <div>
                <p className="text-sm text-ink-muted mb-1">التاريخ</p>
                <p className="font-bold text-ink text-lg">
                  {new Date(confirmedBooking.date).toLocaleDateString("ar-SA")}
                </p>
              </div>
              <div>
                <p className="text-sm text-ink-muted mb-1">الوقت</p>
                <p className="font-bold text-ink text-lg">{confirmedBooking.time}</p>
              </div>
              <div className="sm:col-span-2">
                <p className="text-sm text-ink-muted mb-1">بيانات العميل</p>
                <p className="font-bold text-ink text-base">
                  {confirmedBooking.customerName} - <span dir="ltr">{confirmedBooking.customerPhone}</span>
                </p>
              </div>
            </div>
          </Panel>
          ...
          <div className="pt-8 flex flex-col sm:flex-row gap-4 justify-center">
            <Button size="lg" variant="secondary" onClick={() => router.push("/examples/salon-rawnq/app/services")}>
        ...
        <Panel className="p-0 overflow-hidden border-line">
          ...
          <div className="flex items-center gap-6 bg-surface-elevated border-b border-line px-8 py-5">
            <div className={`flex items-center gap-3 ${step >= 1 ? "text-primary" : "text-ink-muted"}`}>
              <span className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${step >= 1 ? "bg-primary text-white" : "border-2 border-current bg-surface"}`}>
                1
              </span>
            ...
            <div className={`flex items-center gap-3 ${step >= 2 ? "text-primary" : "text-ink-muted"}`}>
              <span className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${step >= 2 ? "bg-primary text-white" : "border-2 border-current bg-surface"}`}>
                2
              </span>
          ...
          <div className="p-8 md:p-10">
            {step === 1 && (
              <div className="space-y-8">
                <div>
                  <h3 className="text-lg font-serif font-bold text-ink flex items-center gap-2 mb-4">
                    <Calendar className="w-5 h-5 text-primary" /> اختر التاريخ
                  </h3>
                  <div className="flex flex-wrap gap-3">
                    {dates.map((date) => (
                      <button
                        ...
                        className={`px-5 py-3 rounded-xl text-sm font-bold border-2 transition-all focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 ${
                          selectedDate === date
                            ? "border-primary bg-primary/5 text-primary"
                            : "border-line bg-surface hover:border-primary/50 text-ink"
                        }`}
                      >
                ...
                {selectedDate && (
                  <div>
                    <h3 className="text-lg font-serif font-bold text-ink flex items-center gap-2 mb-4">
                      <Clock className="w-5 h-5 text-primary" /> اختر الوقت المتاح
                    </h3>
                    <div className="flex flex-wrap gap-3">
                      {availableSlots.length > 0 ? (
                        availableSlots.map((slot) => (
                          <button
                            ...
                            className={`px-6 py-3 rounded-xl text-sm font-bold border-2 transition-all focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 ${
                ...
                <div className="pt-8 flex justify-end">
                  <Button size="lg" onClick={() => setStep(2)} disabled={!selectedDate || !selectedTime} className="w-full sm:w-auto">
            ...
            {step === 2 && (
              <div className="space-y-8">
                <div>
                  <h3 className="text-lg font-serif font-bold text-ink flex items-center gap-2 mb-6">
                    <User className="w-5 h-5 text-primary" /> بيانات الاتصال
                  </h3>
                ...
                <div className="bg-background rounded-2xl p-6 border border-line">
                  ...
                  <p className="text-sm font-medium text-ink-secondary">
                    {new Date(selectedDate).toLocaleDateString("ar-SA")} الساعة {selectedTime}
                  </p>
                ...
                <div className="pt-8 flex flex-col sm:flex-row gap-4 justify-between">
                  <Button size="lg" variant="secondary" onClick={() => setStep(1)} className="w-full sm:w-auto order-2 sm:order-1">
                  ...
                  <Button
                    ...
                    className="w-full sm:w-auto order-1 sm:order-2 shadow-glow"
                  >
                    تأكيد الحجز بنجاح
                  </Button>
                </div>
...
export default function BookingPage() {
  return (
    <Suspense fallback={<div className="py-20 text-center text-ink-muted">جاري تحميل الحجز...</div>}>
      <BookingWizard />
    </Suspense>
  );
}
