...
export function Button({ className = "", variant = "primary", size = "md", children, ...props }: ButtonHTMLAttributes<HTMLButtonElement> & { variant?: "primary" | "secondary" | "outline" | "ghost" | "danger", size?: "sm" | "md" | "lg" }) {
  const base = "inline-flex items-center justify-center rounded-xl font-medium transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 disabled:opacity-50 disabled:pointer-events-none";
  const sizes = {
    sm: "h-9 px-4 text-xs",
...
export function MetricCard({ label, value, icon, change }: { label: string; value: string; icon: React.ReactNode; change?: { value: string; positive: boolean } }) {
  return (
...
export function DataTable<T extends object>({ data, columns }: { data: T[]; columns: { key: string; header: string; render?: (row: T) => React.ReactNode }[] }) {
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-start">
        <thead className="border-b border-line bg-surface-elevated">
          <tr>
            {columns.map(c => (
              <th key={c.key} className="py-4 px-5 text-xs font-bold text-ink-muted text-start whitespace-nowrap uppercase tracking-wider">{c.header}</th>
            ))}
          </tr>
        </thead>
...
export function SectionHeader({ eyebrow, heading, subtitle, align = "start" }: { eyebrow: string; heading: string; subtitle?: string; align?: "start" | "center" }) {
  return (
...
export function AppShell({ children, brandName = "صالون رونق" }: { children: React.ReactNode, brandName?: string }) {
  const location = useLocation();
  const navItems = [
    { label: "الخدمات", path: "/app/services" },
    { label: "حجزي", path: "/app/booking" },
    { label: "الإدارة", path: "/app/admin" }
  ];
      ...
      <header className="sticky top-0 z-40 bg-surface/90 backdrop-blur-md border-b border-line shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
           <div className="flex items-center gap-10">
             <Link to="/" className="font-serif text-3xl font-bold text-primary tracking-tight">{brandName}</Link>
             <nav className="hidden md:flex items-center gap-2">
               {navItems.map(item => {
                  const isActive = location.pathname.startsWith(item.path);
                  return (
                    <Link key={item.path} to={item.path} className={`px-4 py-2 rounded-lg text-[0.9375rem] font-medium transition-all ${isActive ? "bg-primary/10 text-primary" : "text-ink-secondary hover:text-ink hover:bg-black/5"}`}>
                      {item.label}
                    </Link>
                  )
               })}
...
export function LandingShell({ brandName, ctaLabel, ctaTo, children }: { brandName: string; ctaLabel: string; ctaTo: string; children: React.ReactNode }) {
  return (
    <div className="min-h-screen bg-background selection:bg-primary/20 flex flex-col">
       <header className="fixed top-0 inset-x-0 z-50 bg-background/80 backdrop-blur-lg border-b border-line">
         <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
            <Link to="/" className="font-serif text-3xl font-bold text-primary tracking-tight">{brandName}</Link>
            <Link to={ctaTo} className="inline-flex items-center justify-center h-11 px-6 rounded-xl bg-primary text-white font-medium hover:bg-primary/90 transition-colors shadow-sm focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">
               {ctaLabel}
            </Link>
         </div>
       </header>
       ...
       <footer className="border-t border-line bg-surface mt-24">
         <div className="max-w-7xl mx-auto px-6 py-12 flex flex-col md:flex-row items-center justify-between gap-6">
            <Link to="/" className="font-serif text-2xl font-bold text-ink opacity-50 hover:opacity-100 transition-opacity">{brandName}</Link>
...
export function FadeIn({ children, delay = 0, direction = "up", duration = 0.6, className = "" }: { children: React.ReactNode; delay?: number; direction?: "up" | "down" | "left" | "right"; duration?: number; className?: string }) {
  const directions = {
    up: { y: 30, x: 0 },
    down: { y: -30, x: 0 },
    left: { x: 30, y: 0 },
