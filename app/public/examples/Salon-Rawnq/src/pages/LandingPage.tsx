...
export default function LandingPage() {
  ...
  return (
    <LandingShell brandName="صالون رونق" ctaLabel="احجز موعدك" ctaTo="/app/services">
      {/* ── Hero ── */}
        ...
        <div className="grid gap-16 lg:grid-cols-[55fr_45fr] lg:items-center">
          <FadeIn direction="up" duration={0.8}>
            ...
            <div className="mt-12 flex flex-wrap items-center gap-5">
              <Button size="lg" onClick={() => navigate("/app/services")} className="px-10 shadow-glow">
                احجز الآن <ArrowLeft className="h-5 w-5 ms-3" />
              </Button>
              <Button variant="ghost" size="lg" onClick={() => navigate("/app/services")}>
                استكشف الخدمات
              </Button>
            </div>
          </FadeIn>
              ...
              <motion.div key={service.id} variants={staggerItem}>
                <Panel className="h-full flex flex-col md:flex-row gap-6 p-6 hover:shadow-card-hover transition-all border-transparent hover:border-line">
                  <div className="md:w-1/3 aspect-square overflow-hidden rounded-2xl bg-background relative shrink-0">
                    ...
                    {service.isOnOffer && (
                       <span className="absolute top-3 right-3 bg-white text-primary text-[0.65rem] font-bold px-2 py-1 rounded-md shadow-sm">{service.offerLabel}</span>
                    )}
                  </div>
                  <div className="flex-1 flex flex-col justify-center">
                    ...
                    <Button variant="outline" className="mt-auto self-start" onClick={() => navigate(`/app/services/${service.id}`)}>
                      عرض التفاصيل
                    </Button>
                  </div>
                </Panel>
        ...
        <section className="mx-auto max-w-7xl px-6 pb-24">
          <div className="relative overflow-hidden rounded-[3rem] bg-primary px-8 py-20 text-center lg:px-16 shadow-elevated">
            ...
            <div className="relative z-10">
              ...
              <p className="mx-auto mt-6 max-w-xl text-lg text-white/90 leading-relaxed font-medium">احجزي موعدك الآن واكتشفي مفهوماً جديداً للرعاية والجمال في أجواء تليق بك.</p>
              <Button
